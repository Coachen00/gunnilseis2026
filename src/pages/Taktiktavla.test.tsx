...
describe("Taktiktavla", () => {
  ...
  it("matches the supplied corridor, play-zone and thirds geometry", async () => {
    ...
    // Liggande 105×68-plan, anfall mot höger. Korridorerna delar BREDDEN (y),
    // tredjedelar och spelytor delar LÄNGDEN (x). Måtten kommer ur
    // design-systemets pitchGeometry.
    ...
    expect(corridors.map((corridor) => corridor.style.top)).toEqual(["0%", "20.35%", "36.53%", "63.47%", "79.65%"]);
    expect(corridors.map((corridor) => corridor.style.height)).toEqual(["20.35%", "16.18%", "26.94%", "16.18%", "20.35%"]);

    const thirds = Array.from(document.querySelectorAll<HTMLElement>("#layer-tredjedelar .third-band"));
    expect(thirds).toHaveLength(3);
    expect(thirds.map((third) => third.style.left)).toEqual(["0%", "33.3333%", "66.6667%"]);
    expect(thirds.map((third) => third.style.width)).toEqual(["33.3333%", "33.3334%", "33.3333%"]);
    const playZones = Array.from(document.querySelectorAll<HTMLElement>("#layer-spelytor .play-zone"));
    expect(playZones).toHaveLength(4);
    expect(playZones.map((zone) => zone.style.left)).toEqual(["0%", "33.3333%", "50%", "66.6667%"]);
    expect(playZones.map((zone) => zone.style.width)).toEqual(["33.3333%", "16.6667%", "16.6667%", "33.3333%"]);
